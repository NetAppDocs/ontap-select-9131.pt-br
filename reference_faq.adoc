...
* Compatível com ESX e KVM (anterior ao ONTAP Select 9.10.1)
...
.O que é o plug-in do vCenter Deploy?
É possível integrar a funcionalidade ONTAP Select Deploy no servidor vCenter com o plug-in ONTAP Select Deploy. Observe que o plug-in não substitui o ONTAP Select Deploy. Em vez disso, o ONTAP Select Deploy funciona em segundo plano, e o administrador do vCenter pode invocar a maior parte da funcionalidade de implantação do ONTAP Select com o plug-in. Algumas operações de implantação do ONTAP Select estão disponíveis apenas usando a CLI.

.Quantas VMs de implantação do ONTAP Select podem Registrar seus plug-ins com um servidor vCenter?
Apenas uma VM de implantação do ONTAP Select pode Registrar seu plug-in com um servidor vCenter específico.

.Qual é o benefício do plug-in ONTAP Select Deploy vCenter?
O plug-in permite que administradores do vCenter e GENERALISTAS DE TI criem clusters do ONTAP Select usando a GUI do vCenter HTML5. Observe que a GUI do Flash vCenter não é suportada.

Além disso, ele permite que o ONTAP Select Deploy use o vCenter RBAC para autenticação. Os usuários que recebem o privilégio do vCenter de usar o plug-in do ONTAP Select Deploy têm sua conta do vCenter mapeada para o usuário administrativo do ONTAP Select Deploy. O ONTAP Select Deploy Registra o ID de usuário de cada operação e o seguinte arquivo pode ser usado como um log de auditoria básico:
